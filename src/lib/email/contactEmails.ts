// src/lib/email/contactEmails.ts
...
function buildNotificationEmail(data: ContactFormData) {
  const { fullName, email, businessType, projectType, budget, message } = data;
...
function buildConfirmationEmail(data: ContactFormData) {
  const { fullName, email, businessType, projectType, budget, message } = data;
...
export async function sendContactEmails(data: ContactFormData) {
  ...
  // Optional but nice: verify SMTP configuration
  await transporter.verify();

  const notificationEmail = buildNotificationEmail(data);
  const confirmationEmail = buildConfirmationEmail(data);
